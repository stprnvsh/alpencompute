"""Template integration for onboarding partner-provided bare-metal servers."""
...
_SAMPLE_SERVERS: List[Dict[str, Any]] = [
    {
        "serial_number": "EXAMPLE-GPU-01",
        "type": "h100x8-nvswitch-2tb",
        "datacenter": "ch-zrh-1",
        "rack": "row-7",
        "bmc_endpoint": "https://bmc-gpu-01.example.com",
        "bmc_username": "admin",
        ...
        "nic_speed_gbps": 100,
        "power_state": "off",
        "hostname": "gpu-training-01",
    }
]


class PartnerAPIClient:
    """Minimal async wrapper around a partner bare-metal API or static manifest."""
    ...
    def __init__(
        ...
        *,
        ...
        for server in self._resolve_inventory(sample_inventory, inventory_file):
            self._inventory[server["serial_number"]] = dict(server)

    def _resolve_inventory(
        self,
        sample_inventory: Optional[List[Dict[str, Any]]],
        ...
        if inventory_file:
            ...
            if not isinstance(data, list):
                raise ValueError("Inventory file must contain a JSON array of server definitions")
            logger.info("Loaded %s servers from %s", len(data), inventory_file)
            return data
        if sample_inventory:
            return sample_inventory
        logger.info(
            "Using built-in sample inventory for partner %s; set PARTNER_INVENTORY_FILE to override",
            self._partner_name,
        )
        return _SAMPLE_SERVERS

    async def list_servers(self, *, region: Optional[str] = None) -> List[Dict[str, Any]]:
        servers = await self._fetch_inventory()
        if region:
    ...
    async def get_server(self, serial: str) -> Dict[str, Any]:
        servers = await self._fetch_inventory()
        return next(server for server in servers if server["serial_number"] == serial)
    ...
    async def set_boot_device(self, serial: str, device: str) -> None:
        if self._session is None:
            ...
            return
        await self._request("POST", f"/servers/{serial}/actions/boot", json={"device": device})
    ...
    async def _fetch_inventory(self) -> List[Dict[str, Any]]:
        ...
        payload = await self._request("GET", "/servers")
        if not isinstance(payload, list):
            raise ValueError("Expected the partner API to return a list of servers")
        return payload

    async def _request(self, method: str, path: str, **kwargs: Any) -> Any:
        if self._session is None:
            raise RuntimeError(
                "HTTP session is only available when both PARTNER_API_BASE and PARTNER_API_TOKEN are provided"
            )
...
class PartnerManagementDriver(BaseManagementDriver):
    """Adapter between a partner API client and the :class:`CustomerHardwareController`."""

    def __init__(self, client: PartnerAPIClient) -> None:
        self._client = client
...
async def load_inventory(
    ...
    *,
    ...
    logger.info("Loaded %s partner servers into the controller", len(servers))


async def main() -> None:
    logging.basicConfig(level=logging.INFO)
    partner_name = os.getenv("PARTNER_NAME", "Example Partner")
    site_name = os.getenv("PARTNER_SITE_NAME", f"{partner_name} Site")
    contact_email = os.getenv("PARTNER_CONTACT_EMAIL", "ops@example.com")
    region = os.getenv("PARTNER_REGION", "ch-zrh-1")
    warm_pool_target = int(os.getenv("PARTNER_WARM_POOL_TARGET", "1"))
    mesh_endpoint = os.getenv("MESH_ENDPOINT", "mesh.alpencompute.local:51820")
    inventory_file = os.getenv("PARTNER_INVENTORY_FILE")
    api_token = os.getenv("PARTNER_API_TOKEN")
    api_base = os.getenv("PARTNER_API_BASE")

    client = PartnerAPIClient(
        partner_name=partner_name,
        token=api_token,
        base_url=api_base,
        sample_inventory=_SAMPLE_SERVERS,
        inventory_file=inventory_file,
    )
    driver = PartnerManagementDriver(client)
    ...
    await run_agent_for_duration(agent, seconds=int(os.getenv("PARTNER_AGENT_DURATION", "30")))
